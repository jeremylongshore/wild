...
- **Closed the two_phase guard-chain bypass, made nonce denial responses opaque, and merged policy defaults into every action** (review wave, findings f-l10-4, f-l10-5, f-l10-6). `AuditedPipeline` no longer delegates `:two_phase` and `Guard::Pipeline#two_phase` is now private, so nothing holding a pipeline reference can reach `TwoPhaseFlow#confirm_and_execute` directly and skip the allowlist, rate limit, blast-radius check, and audit record (`#call` is the only sanctioned entry point). `ResponseFormatter#denied_hash` drops `:internal_reason` from the client-facing body (it stayed inside a splat despite `NonceManager`'s own opaque-failure doc comment, so not_found/expired/already_used/mismatch were distinguishable to the caller); `Recorder` now prefers `internal_reason` for the audit trail's `denial_reason` so the granularity moves server-side instead of disappearing. `PolicyConfig` merges the required `defaults` section into each action hash at load time (explicit per-action keys still win) instead of only validating it and never using it, so an action that omits `rate_limit` or `blast_radius_cap` inherits the default instead of crashing `RateLimiter`/`BlastRadiusEnforcer` with `NoMethodError`/`ArgumentError` the first time it runs. Advances bead "F6: Audit and either wire or delete every half-published API surface across the namespaces".
