...
- **Review-proven package claims, wildcard invariants, and vanity specs corrected**
  ...
  dependencies and behavior exercised by the code; the unused CapabilityGate
  to Permission edge is removed. The wildcard corpus's negative-case invariant
  and its one wildcard-detection grammar are executable specs. The affected
  top-level smoke tests now exercise public behavior rather than module/version
  trivia. `Wild.config.capability_gate.capabilities_path` is explicitly
  documented as unwired until Role 7 integration instead of being presented as
  live configuration.
...
- **Filled the `config.after_initialize` hook so central configuration reaches `Wild::AdminTools` and `Wild::Introspection` at boot** (review wave, findings f-l10-3, f-x1-1, f-l09-2; the F1 Configuration-bridging regression). The hook was comment-only, so `Wild.config.admin_tools`/`Wild.config.introspection` were read by nothing at runtime: the `:default` adapter sentinels never resolved (every admin action died on a `nil` adapter, `Wild::AdminTools::Configuration#validate!` had zero callers), and following the README's `Wild.configure { |c| c.introspection.access_policy_path = ... }` example had no effect on the actual runtime policy loader (a separate, never-synced singleton), so every introspection call raised `ModelNotAllowedError` regardless. The hook now calls two new module methods, `Wild::AdminTools.bridge_configuration!` and `Wild::Introspection.bridge_configuration!` (defined in each namespace's own `lib/wild/<namespace>.rb` entry file, not in `engine.rb` itself: per ADR-0003 the root package's `package.yml` declares zero namespace dependencies, so the resolution/adapter-wrapping logic has to live inside each namespace, with the engine only calling its public entry point (verified with `bundle exec packwerk check`, still 0 offenses). `AdminTools.bridge_configuration!` resolves `cache_adapter`/`job_adapter`/`flag_adapter` `:default` sentinels to `Wild::AdminTools::Executor::Adapters::{RailsCacheAdapter,SidekiqAdapter,FlipperAdapter}` when the matching backend (`Rails.cache`, `Sidekiq`, `Flipper`) is loaded, copies through any adapter the consumer set explicitly, and calls `#validate!`, which raises `Wild::ConfigurationError` naming the setting if it's still unresolved (chose failing loudly at boot over letting a misconfigured admin surface raise `NoMethodError` on first use, since Sidekiq/Flipper aren't gemspec dependencies, and most Rails apps mounting this engine won't have them installed and must configure `job_adapter`/`flag_adapter` explicitly, as `spec/dummy/config/application.rb` now does). `Introspection.bridge_configuration!` is a no-op when the central `access_policy_path` was never set, otherwise pushes it and `blocked_resources_path` (new field on `Wild::Configuration::Introspection`, needed for `Wild::Introspection::Configuration#load!` to succeed) into the runtime loader and calls `#load!`. README's Configuration section updated to reflect both paths being required together and that only Sidekiq (not `ActiveJob::Base`, which has no generic queue-introspection API to wrap) is a shipped `job_adapter` backend. Closes beads "Resolve Wild.config.admin_tools.{cache,job,flag}_adapter :default sentinels at Wild::Engine after_initialize", "Wire Wild.config.introspection.access_policy_path into the Wild::Introspection::Configuration policy loader at engine boot", "Reconcile Wild::AdminTools::Configuration (nil-default injection points) with the central Wild::Configuration::AdminTools :default sentinels at engine boot".
...
- **Made nonce consumption atomic and loaded the three concrete adapters** (review wave, findings f-l10-1, f-l10-2, plus same-file ride-alongs f-l10-9 and f-l10-14). `Guard::NonceManager#validate_and_consume!` was check-then-act across three separate `NonceStore` mutex acquisitions (fetch, test `entry.consumed`, then `consume!`), so N concurrent confirms of one nonce could all observe `consumed == false` before any of them consumed it, double-executing whatever `mutate_destructive` action the nonce was gating; `NonceStore#consume_if_unconsumed!` now performs the "not already consumed" check and the consume inside one mutex acquisition, closing the race (verified with a 16-thread barrier-raced spec run 50 times, plus `--seed 1`/`--seed 41803`). The three concrete adapters (`RailsCacheAdapter`, `SidekiqAdapter`, `FlipperAdapter`, 291 LOC, the entire "works with Rails.cache/Sidekiq/Flipper" story) were never required from `lib/wild/admin_tools.rb`, so referencing them raised `NameError` and they were invisible to coverage; now required (each still lazily `require`s its own gem only inside the methods that need it, so the gem keeps loading without sidekiq/flipper installed). Fixed the same-file `RailsCacheAdapter#list_keys(prefix:, ...)` arity mismatch against the abstract `CacheAdapter#list_keys(**_options)` contract (`prefix` now defaults to `""`), and closed the sweep-thread spawn race in `NonceStore` (the `unless @running` guard now lives inside `@mutex`, and `stop_sweep!` wakes the thread immediately via a `ConditionVariable` instead of waiting up to 10s for its `sleep` to elapse). Added a structural "an admin tools adapter" shared example (keyword-arity compatibility with the abstract base) applied to all three concrete adapters and the three spec-support test doubles, plus a behavioral spec per concrete adapter against a fake backend (Hash-backed `Rails.cache` double, Sidekiq-shaped `DeadSet`/`Queue` fakes, a Flipper-shaped `Registry`/`Feature` fake). Advances bead "Move wild-admin-tools-mcp into Wild::AdminTools and kill the DI container".
...
- **Wildcard detection centralized into `WildcardMatcher.wildcard?`, compiled Regexp cached, corpus non_matches invariant enforced** (review wave, findings f-l05-1, f-l05-2, f-l05-3). `WildcardMatcher.matches?` compiled a fresh `Regexp.new` on every call (finding's own benchmark: ~3.3s vs ~0.17s for 300k calls of the same pattern, ~20x); `matches?` now compiles through a bounded, mutex-guarded per-pattern cache. Chose a class-level bounded `Hash` over precomputing once per `Report::Builder#build` because the cache is correct and cheap across every call site, not just the batch-analysis path. Wildcard detection itself (`pattern.include?("*")`) was duplicated verbatim at 6 sites (`orphan_analyzer`, `wildcard_matcher`, `risk_analyzer`, `shadow_analyzer`, `consistency_analyzer`, `models/grant`); all 6 now delegate to the new `WildcardMatcher.wildcard?`, so the wildcard grammar lives in one module. `spec/wild/schemas/wildcard_corpus_spec.rb` asserted `non_matches` is an Array but never that it is non-empty for non-universal patterns, despite the corpus YAML's own header comment documenting that invariant in prose; now enforced as a real assertion. Closes bead "Centralize wildcard detection (include?('*')) into WildcardMatcher.wildcard? so the grammar lives in one module" (`wild-yms`) and bead "Enforce the wildcard corpus non_matches-non-empty-except-universal invariant in the schema spec instead of a comment" (`wild-96t`).
