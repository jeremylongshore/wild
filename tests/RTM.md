...
| REQ-006 | Downgrade `Wild::Skillops` claims to match what code can back up (no atomicity/durability claims) | F5 | wild-rvv.8.1 | `spec/wild/skillops/registry/store_spec.rb` (class comment truth check) | **Partial — `registry/store.rb` comment now states actual guarantees (review wave f-l07-1/f-l07-2); `Wild.config.skillops.enabled` gating a no-op (f-l07-3) is still open, owner decision pending** |
