...
RSpec.describe "Guard::Pipeline end-to-end lifecycle" do
  ...
  let(:pipeline) do
    Wild::AdminTools::Guard::Pipeline.new(policy_config: valid_policy_config).tap do |p|
  ...
  context "when nonce is expired" do
    it "returns denied with reason nonce_invalid" do
      ...
      entry = pipeline.send(:two_phase).nonce_manager.store.fetch(nonce)
      ...
      pipeline.send(:two_phase).nonce_manager.store.instance_variable_get(:@mutex).synchronize do
        pipeline.send(:two_phase).nonce_manager.store.instance_variable_get(:@entries)[nonce] = expired_entry
      end
